anew opengl.f   \ May 17th, 2008 by Jos v.d.Ven Version

: OldVersion?   ( version# - )
     61200 < abort" You need Win32Forth version 6.12 or better." ;

version# OldVersion?

(( Short manual:

compile _AllScenes.f or _Study.f to get started.

The idea is to make openGL available for Win32Forth with a number
of additional functions.

Change a parameter of a function to change a scene.
When a function is changed the affected parameters can be shown in the
title of the window.

You can change the active function by hitting a key or choose
the function from a menu.

List of keys of the intended functions (Don't work in the NeHe lessons):

Key  functions in view
-----------------------
  C  Change center-offset of the eye
  D  Change the direction of the eye
  E  Move the viewpoint of the eye
  F  Change up-offset of the eye ??
  G  Rotate cylinder using glRotatef

  H  Change the size of an outlined cube
  I  Move cylinder ( object ) using glTranslatef
  ...
  X  Rotate around the center

  =  Increments the distance to move
  -  Decrements the distance to move

Key  functions in light
-----------------------
  L  Move light
  N  Ambient
  V  Diffuse

Key  functions in Options
-------------------------
W - Full screen or Window
Z - Rotate a static scene
F1 - Change full screen to window

HOME  Reset active function
ESC   Stop. This enables you to cut and paste the parameters into your source

To change a PARAMETER of the ACTIVE function you will have to press one
of the following keys:

Key -*stack          action while moving
----------------------------------------
RIGHT   3            Right.
LEFT    3            Left.
UP      2            Up.
DOWN    2            Down.
PGUP    1            Zoom out.
PGDN    1            Zoom in. Forwards is DOWN in numbers on the negative z-axis!
.       4            Rotate right. Ignored when not needed.
,       4            Rotate left.  Ignored when not needed.

Note: -*stack is the element on the floating point stack which will be changed.

Example:
<r> means hit the key with the r

If you would like to rotate the scene push the <r> and then the <.>
Note: The angle is the fourth element on the floating point stack and
      will be changed. You can copy them from Win32Forth when you hit <Escape>

Try the following:
Move the cylinder towards you by using the keys <o> and then <PageDown> until
you don't see it.
Then rotate the eye 360 degrees by using the <r> and then the <.> key.
Most of these functions also work while the ball is bouncing.

The keys can be used in full screen mode or window mode.

When you see the following line in a source:
.00100e .02000e 1.3830e fref3Trans1 floatsf@+  glTranslatef

The following happens:
It adds the offsets from a floating-point array fref3Trans1 to the floats
on the floating point stack by using floatsf@+ then
perform the openGL function glTranslatef.
fref3Trans1 can interactively be changed.

fref3Trans1 provides the address of its array and size on the stack.
The parameters which might be changed are already on the floating point stack
The stack notation of floatsf@+ is:
( adr k - ) ( f: fk..f0 - fk+floatsk..f0+floats0 )

A Note for Window 95:
If your graphic card does not support OpenGL download OPENGL95.EXE
from Microsoft and run it.
Copy glu32.dll and Opengl32.dll to your c:\windows\system
Then start Win32Forth and fload _AllScenes.f or _Study.f again.


When you resize the window while an animation is going on the
animation will disappear and stop.
...
- Added ScaleDib to increase speed greatly :-) ( try .3 )
  Downside: ScaleDib decreases the quallity of the image in the window.
))
...
needs gldefs.f     \ Opengl constants.
needs bmpio.f
...
\in-system-ok  if   postpone ['] compile (+fto
...
\ *foo means an absolute adres to an array.
\ It's size depend on the used function and how it is used.
...
: glGenTextures   ( GLsizei *name -- )          swap call glGenTextures    drop ;
...
: init-DIBsection ( - )
     ...
     BitBltMode
      if    width @ height @
      else  Stretchwidth @ Stretchheight @
      then
...
: glout
        ghrc 0 <> if release-context                                  then
     hbitmap 0 <> if hbitmap    Call DeleteObject drop
                     hdc-pixmap call DeleteDC drop 0 to hbitmap       then
 ;

: limit-bpps  ( bpps - )  to _limit-bpps glout init-DIBsection  ;
...
: floatsf@+ ( adr k - ) ( f: fk..f0 - fk+floatsk..f0+floats0 )
   dup 1- swap 0
      do    dup i - floats 2 pick + f@ dup 1+ froll f+
...
:  zx ( - adr-flookat ) ( F: - f ) fref3D 2 addr#floats+@ distance  f+ ;
: -zx ( - adr-flookat ) ( F: - f ) fref3D 2 addr#floats+@ distance  f- ;
:  zy ( - adr-flookat ) ( F: - f ) fref3D 1 addr#floats+@ distance  f+ ;
: -zy ( - adr-flookat ) ( F: - f ) fref3D 1 addr#floats+@ distance  f- ;

:  zz ( - adr-flookat ) ( F: - f ) fref3D 0 addr#floats+@ distance  f+ ;
: -zz ( - adr-flookat ) ( F: - f ) fref3D 0 addr#floats+@ distance  f- ;
...
: _relative-direction  ( adr n - ) ( f: fsum - rel fsum )
   >f@ fover f/0 fswap ;

: relative-direction ( fref4RotD n - ) ( f: - x y z )
   over >r 1 - floats@ f+ f+
   r@ 2 _relative-direction
   r@ 1 _relative-direction
   r> f@ fswap f/0
 ;

\ 27e 1e 2e 3e fref4RotD 2dup floats! relative-direction f.s abort

: dist_f*_fref3r-dir ( - fref3r-dir )  ( f: f - f*dist )
   distance f* fref3r-dir drop  ;

: frot_eyeD  ( f: f1 f2 f3 - f2 f3 f1 _eyeD )
   frot _eyeD ;

: frad   ( fs: deg - rad )   fpi f* 180 s>f f/ ;

\ move_forward and move_backward need to be fixed
: move_forward  ( fref4RotD n fref3TransT n - )
  2>r 2dup 3 addr#floats+@ drop fdup frad fto _eyeD f0=
  if  2drop 2r> 0 addr#floats+@ distance  f+ f! exit
  else
  2dup relative-direction fref3r-dir floats!
   2r@ floats@
   frot_eyeD fsin dist_f*_fref3r-dir 1 >f@ f* f-    \ x dir
   frot_eyeD fcos dist_f*_fref3r-dir 2 >f@ f* f+    \ y dir
   frot_eyeD fcos distance f* f+                    \ z dir
   2r> floats! 2drop display-it
  then
 ;

: move_backward  ( fref4RotD n fref3TransT n - )
  2>r 2dup 3 addr#floats+@ drop fdup frad fto _eyeD f0=
  if  2drop 2r> 0 addr#floats+@ distance  f- f! exit
  else
  2dup relative-direction fref3r-dir floats!
   2r@ floats@
   frot_eyeD fsin dist_f*_fref3r-dir 1 >f@ f* f+
   frot_eyeD fcos dist_f*_fref3r-dir 2 >f@ f* f-
   frot_eyeD fcos distance f* f-
   2r> floats! 2drop display-it
  then
 ;
...
: roll-fref4RotR  ( - )
   1 +to #turns
   Mut4RotR fref4RotR fm+
 ;

0 value resizing?

defer resize_scene
' noop is resize_scene
...
: stop-request ( - )
    h_ev_slow-action event-reset true to request-to-stop ;

: stop_get_params  ( - )
    stop-request winpause title$ count type cr -1 to #turns ;
...
: InterruptAnimation  ( - )
  stop-request glout init-DIBsection
 ;

' InterruptAnimation is resize_scene
...
\                   fref4RotD fref3TransT move_forward  endof  \ move forwards
\     ascii B    of ['] fref3TransT is-fref3D
\             fref4RotD fref3TransT move_backward endof \ move backward
     ...
     VK_F1      of fullscreen-to-window           endof
...
: key-event ( VK_key - )  \ From the window
    dup K_ESC =
      if    ignore_esc
              if   exit
              else true to key-ready? stop_get_params
              then
...
: buffer>screen   ( - )
   \ hdc-pixmap SwapBuffers    \ Not needed rendering is directly done in the DIBsection
    glFinish SRCCOPY BitBltMode
      if    0 0 hdc-pixmap height @ width @  0 0 ghdc call BitBlt
      else  Stretchheight @  Stretchwidth @  0 0 hdc-pixmap height @ width @  0 0 ghdc call StretchBlt
      then
    drop
 ;

: StableBuffer>Screen ( - )
   winpause wait/restart-timer
   1 +to #frames buffer>screen
 ;

' StableBuffer>Screen is show-buffer

: ScaleSize ( n - scaled ) ( f: ScaleRate - ) s>f fover f* f>s 5 max ;

: ScaleDib ( f: Rate - )
   width @ ScaleSize height @ ScaleSize 2dup Stretchheight ! Stretchwidth !
   to heightViewport to widthViewport
   false to BitBltMode
   glout  init-DIBsection glin
 ;

: show-and-save-buffers ( - ) \ init-save$ #frames-done and #frames-to-do must be set
   buffer>screen              \ get an image from OpenGL and put it on screen
   init-save$ count  save$ place    \ reset file-name
    ...
    #frames-done  #frames-to-do >
       if  ['] buffer>screen is show-buffer              \ stop saving when done
            -1 to #turns
...
\ show-frame allows windows prevents an overflow of the keybuffer
...
: FreeTextures ( - )
    Texture 0<>
      if  Texture free 0 to Texture              \ Free when not free
      then
 ;
...
struct{ \ box_dyn
    b/float  ftz       \ Position ( Translatef )
    b/float  fty
    b/float  ftx
    b/float  fmz       \ distance to move
    b/float  fmy
    b/float  fmx
    b/float  frz       \ Rotatef
    b/float  fry
    ...
    b/float  fbox_y
    b/float  fbox_x
    b/float  fbox_hz     \ sizes in relative matrix ( 2/ )
    b/float  fbox_hy
    b/float  fbox_hx
}struct box_dyn
...
: box_sizes!    ( adr-struct-box - ) ( f: width height depth -  )
   dup fbox_z 3 2dup floats! floats@
...
ms-time-out-done #cycles-1-second value old-ms-time-out-done
...
: set-speed-degrees ( - )
    #frames-to-do$ dup ms-slow-action #cycles-1-second val>$
...
: show-#bitmaps
   ...
   max-frames? 1+ 1
     do
       ...
       _load-bitmap
           if  h_ev_timeout_done event-reset
               4th>title save$ count +title ogl-hwnd retitle-window  \ update the title of the window
               winpause  h_ev_timeout_done event-wait
               leave-on-stop
           then
     loop
