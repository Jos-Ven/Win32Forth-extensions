anew -opengl.f   \ April 26th, 2013 by Jos v.d.Ven
...
- Stablized dynamic scenes to +/- 60 FPS by using a waitable timer
...
: DC>screen   ( - )  \ Renderes the DC directy to a window.
...
\ show-frame an overflow of the keybuffer
: show-frame  ( - )
   key?
     if  key drop
     then
   HOpenGLTimer WaitHndl   \ Wait for the pending timeout
   ns-time-out-done HOpenGLTimer StartTimer  \ Start a new timeout
   show-buffer winpause
 ;
...
: GLFunction: \ CompileTime: (  -- <name> )  \ Map: ProcPointer lastext FunctionName ( case sensitive )
   ...
   "header dovar compile,                    \ Create a header from the input stream in uppercase
...
ns-time-out-done f>s ns-1-period  f>s value old-ms-time-out-done
