...
: start-FullscreenOpenGLWindow  ( -- )   true to key-ready? Start: FullscreenOpenGLWindow ;

: end-FullscreenOpenGLWindow    ( -- )
  true to key-ready? stop-request Close: FullscreenOpenGLWindow
  true to resizing?
   static-scene
    if  painting
    then
