...
:Object Form_search_path                <Super Child-Window
...
:M On_Init:     ( -- )
     ...
     s"  \n Choose at least one directory structure containing music files.\n A good structure is c:\MyMusic\Style\Artist\Album\Song.wma\n In this case you should choose: c:\MyMusic as the search path.\n "
     drop 1 -  GetWindowRect: Self 2drop 90 -
     Start:  InfoWindow
                ;M

: AskFolder ( - adr cnt )
        z" Add to search path:"
        temp$ GetHandle: self  BrowseForFolder drop
        temp$ dup +null count
...
: AddToPath  ( str cnt - )
   dup 0>
        if      (SearchPathCatalog "path+
        else    2drop
        then
 ;

: SaveSearchPath ( -- )
   (SearchPathCatalog  off
   ...
   (SearchPathCatalog count vadr-config SearchPathCatalog place
   DirlistToCmbList
   Close: Self
 ;
