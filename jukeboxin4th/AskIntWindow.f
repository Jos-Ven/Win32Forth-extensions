...
:M On_Init:     ( -- )
                ...
                s" Ok" SetText: Bt_ok
                GetStyle: Bt_ok BS_DEFPUSHBUTTON OR SetStyle: Bt_ok
...
:M SetSpinner:   ( - ) \ does not select the value in the spinner
                SetFocus: Spinner1
               ;M
...
: ForAskedInteger  ( parent s"title" cnt offset cfa-ok - )
        Start: AskIntWindow
        SetSpinner: AskIntWindow ;

\s Use:

 0 s" test" 20  ' . ForAskedInteger  \s Press OK and see the value in the console.
