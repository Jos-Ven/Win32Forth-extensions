anew mShellRelClass.f        \ February 17th, 2010  for Win32Forth Version: 6.14.00
...
\ Characteristics:
\ This version uses relative addresses as pointers. So there is no need
...
\ The sort is case-insensitive for strings.
\ Easy to expand to sort doubles etc.
\ ShellSort is a class so it can handle more than 1 database or indexfile
...
:CLASS  ShellSort    <Super Object

      int record-size
      int #records
      int aptrs  \ an array of cells containing pointers to records
      int records-pointer


:M ClassInit:    ( -- )        ClassInit: super             ;M

:inline n>aptr   ( n -- a_to_ra_in_idx )  aptrs +cells      ;
...
:M n>record:      ( n -- a )  n>record ;M
:M >record:       ( n -- a )  >record  ;M
:M n>aptr:        ( n -- a_to_ra_in_idx ) n>aptr ;M
...
:M #records-in-database: ( m_hndl - #records )  >hfileLength @ record-size /   ;M
...
: FlushCloseFile ( hndl - )   dup flush-file drop close-file throw ;

: create-file-ptrs ( name -- )
   count r/w create-file abort" Can't create index file."  FlushCloseFile
 ;
